...
- Added `Server.on_shutdown(callback)` (also on `RiffqServer`): a zero-argument
  callback invoked once when the server shuts down. riffq now catches **SIGTERM**
  in addition to SIGINT inside its Rust runtime and runs the callback before
