...
runtime, stops accepting connections, then calls your zero-argument callback
before `start()` returns. Use it to flush or checkpoint state for a clean exit.
...
`on_shutdown` avoids that entirely.
