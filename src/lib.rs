...
impl PythonWorker {
            ...
            loop {
                ...
                match rx.recv() {
                    Ok(msg) => match msg {
                        ...
                        WorkerMessage::Disconnect { connection_id } => {
                            if let Some(cb) = disconnect_cb.lock().unwrap().clone() {
                                Python::with_gil(|py| {
                                    let args = PyTuple::new(py, &[connection_id.into_py(py)]);
    ...
    pub async fn on_disconnect(&self, connection_id: u64) {
        let _ = self
            .sender
            .send(WorkerMessage::Disconnect { connection_id });
    }
}

pub struct DummyProcessor {
    py_worker: Arc<PythonWorker>,
    connections: Arc<Mutex<HashMap<SocketAddr, u64>>>,
}
...
impl NoopStartupHandler for DummyProcessor {
        ...
        PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
    {
        ...
        self.connections.lock().unwrap().insert(addr, id);
        let allowed = self
            .py_worker
...
pub struct MyExtendedQueryHandler {
    ...
    pub connections: Arc<Mutex<HashMap<SocketAddr, u64>>>,
}
#[derive(Clone)]
...
impl Server {
    ...
    fn run_server(&self) {
        ...
        let on_query_cb = self.on_query_cb.clone();
        let on_connect_cb = self.on_connect_cb.clone();
        let on_disconnect_cb = self.on_disconnect_cb.clone();
        let connections = self.connections.clone();

        if on_query_cb.lock().unwrap().is_none() {
        ...
        rt.block_on(async move {
            let py_worker = Arc::new(PythonWorker::new(on_query_cb, on_connect_cb, on_disconnect_cb));

            let factory = Arc::new(DummyProcessorFactory {
                handler: Arc::new(DummyProcessor { py_worker: py_worker.clone(), connections: connections.clone() }),
                extended_handler: Arc::new(MyExtendedQueryHandler { py_worker: py_worker.clone(), connections: connections.clone() }),
            });
            ...
            let server_task = tokio::spawn({
                ...
                async move {
                    loop {
                        ...
                        if let Some(mut socket) = detect_gssencmode(socket).await {
                            let addr = socket.peer_addr().unwrap();
                            let factory_ref = factory.clone();
                            let py_worker_cloned = py_worker.clone();
                            ...
                            tokio::spawn(async move {
                                ...
                                let id_opt = {
                                    connections_cloned.lock().unwrap().remove(&addr)
                                };
                                if let Some(id) = id_opt {
                                    py_worker_cloned.on_disconnect(id).await;
                                }
                            });
                        }
                    }
